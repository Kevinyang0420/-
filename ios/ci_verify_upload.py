...
MAX_AGE_MIN = 90
#: 苹果入库有延迟，轮询几次再判。
# 🚨🚨 **10×30秒＝5分钟太短**（2026-08-28 实撞）：构建 619 上传成功，
#    5 分钟没出现 → 这条判成 FAIL、整个 run 红；**而它在约 40 分钟后自己出现了**。
#    苹果的入库时间不受我们控制，5 分钟只是我拍的。
#    **假阴性跟假阳性一样坏**：闸门天天红，人就会开始无视它，
#    等到真的没上去那天也不会有人当回事。
#    改成 45×45 秒 ＝ **约 34 分钟**（619 实测约 40 分钟才入库，所以下限按它定）。
#    MAX_AGE_MIN 同步抬到 90 —— 它防的是"拿几小时前的旧包冒充"，
#    90 分钟仍然远小于今晚任意两次发版的间隔，那道防线不受影响。
# 🚨🚨 **等待时长要比已知的实际耗时长，不是短。**
#    2026-09-06：这里原来是 45×45 = **33 分钟**，而同一份 workflow 的注释
#    白纸黑字写着「实测 619 花了约 **40 分钟**」——
#    **判据的阈值比自己记录的事实还短**，于是上传明明
#    `UPLOAD SUCCEEDED with no errors`（Delivery UUID 都有了），
#    闸门却报「App Store Connect 上没有出现刚上传的构建」，
#    还顺手把人引去查 altool ——**一个误报，把排查带向了完全无关的方向**。
#    70×45 = 52 分钟，配合 job 的 90 分钟上限还有余量。
#    🚨 `MAX_AGE_MIN` 不动：它防的是"拿几小时前的旧包冒充"，跟等多久无关。
TRIES = 70
WAIT = 45
