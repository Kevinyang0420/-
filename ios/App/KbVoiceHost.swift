...
final class KbVoiceHost {
    ...
    private func tick() {
        ...
        //    表现是用着用着突然又开始跳转。
        // 🚨🚨🚨 **09-13 查出的回归：这段自愈+180s闲置放手，被锁在 `holdIsPlayRec` 后面。**
        //    这段代码写的时候保活还是"方案B"(.playAndRecord)默认开，
        //    09-03 把默认改回 `.playback`（`holdIsPlayRec` 现在默认 `false`）之后，
        //    整段（含 `markArmed` 续期、tap 自愈、180s 后放手灭灯）**在默认配置下从没跑过**。
        //    两个后果都对得上 Kevin 09-13 报的现象：
        //      ①「标记说架好了但连续几次录不上，必须杀 App」——续期/自愈没在跑；
        //      ②「静默播放一晚 100%→13%」——180s 闲置放手没在跑，
        //        保活能一直播到 `standbyLimit`（24 小时）才停。
        //    这段代码本身跟具体用哪个 AVAudioSession 分类无关（`hold.start()`
        //    自己会去读 `holdIsPlayRec` 决定配 `.playAndRecord` 还是 `.playback`），
        //    所以把外层这道分类闸拿掉，让它两种模式下都跑。
        do {
