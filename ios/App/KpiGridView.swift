...
final class KpiGridView: UIView {
    ...
    private func cell(_ v: (num: String, unit: String, label: String),
        ...
        if let note = note {
            ...
            // 🚨 **这两行现在基本是空转，留着但要说清楚**（0 2026-09-06 指出）：
            //    iOS 上缩字号**只在单行时可靠**，`numberOfLines = 0` 之后
            //    换行已经接管了。留着是为了极窄屏上单词本身放不下时还有一手，
            //    **但别再把它当成防截断的主要手段** —— 主要手段是下面那行。
            //    （原注释写的是「允许缩到 0.8 倍…截断的脚注比没有更糟」，
            //     那描述的是一个多行下不再发生的机制。**别让注释描述不存在的东西**
            //     —— 今天已经被这种注释坑过一次：`KbVoiceHost:4242` 那句
            //     「跟安卓同一个数」，而安卓连那个常量都没有。）
            n.adjustsFontSizeToFitWidth = true
            n.minimumScaleFactor = 0.8
            // 🚨 **还要允许折两行**（2026-09-06 德文界面实拍截断）。
            //    只有"缩字号"这一手时，德文缩到 0.8 仍然放不下，
            //    结果是「Basierend auf durchschnittlicher Tip…」——
            //    上面那条注释说得对（截断的脚注比没有更糟），
            //    但**它给的手段只够治英文**：德语复合词天生比英文长一截。
            //    加语言的时候，按旧语言调好的余量会悄悄不够用。
            //
            // 🚨🚨 **两行也不够**（0 在我 12:59 的整屏图里抓到，结尾还是 `…`，
            //    少了 `pro Minute`）。而我 12:44 报"已修"用的是一张**裁过的特写**，
            //    **裁掉的正好是要证明的那一段** —— 那不是证据，是自我安慰。
            //
            // 🚨 **不要再猜几行够** —— 猜 2 行错了，猜 3 行下一门语言又会错。
            //    `0` = 要几行给几行。卡片下半截本来就空着，位置有的是。
            //    **判据从"够不够"换成"不设限"，这类问题就结构性地没有了。**
            n.numberOfLines = 0
