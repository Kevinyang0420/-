...
enum Backend {
    ...
    static func lookup(_ raw: String,
        ...
        let body: [String: Any] = [
            ...
            "temperature": 0.2,
            // 🚨🚨 **1200，不是 420**（09-07 端到端撞出来的）。
            //    420 是按**词卡**估的（3 条义项 + 例句 + 搭配）。
            //    查句子上线后，同一个端点也要回句子卡
            //    （meaning + breakdown + alternatives + keys），420 装不下 →
            //    返回被截断成半截 JSON → 解析失败 → 他看到「出了点问题，再试一次」。
            //
            //    🚨 **探针全绿而真机走不通**：`probe_sentence_lookup.py` 用的是
            //    2200，跟客户端不同源 —— 探针验的是后端，验不到客户端的预算。
            //    这类错只有端到端才看得见。
            //
            //    🚨 抬上限**不等于每次都花这么多**：`max_tokens` 是天花板不是目标，
            //    计费按实际输出。查单词仍然只会吐词卡那么长。
            "max_tokens": 1200,
