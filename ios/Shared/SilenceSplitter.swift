...
final class SilenceSplitter {
    ...
    /// 🚨🚨 **「麦克风到底在不在采」的线，跟「他有没有说话」不是同一条。**
    ///
    /// 2026-08-29 定案（0 拍板）：上传前那道闸改成只回答
    /// **「麦克风在工作吗」**，「有没有说话」交给 ASR（空结果 → `asr_empty`）。
    ///
    /// ```
    /// 麦没在采（没权限/被占/没接上）  ≈ 0.000x
    /// 麦在采、只是很安静              ≥ 0.03（他房间实测底噪）
    /// ```
    /// 差三倍以上，所以 **0.01** 两边都有余量。
    /// 而原来的 `0.08` 卡在「底噪 0.03–0.06」和「说话 0.1464」之间，**两边余量都不到一倍**
    /// —— 他小声说话或手机放远一点就会被判成静音。
    ///
    /// 🚨 **同一个数字，换个问题问，余量从 1.3 倍变成 3 倍。**
    ///    不是把线调松了，是这条线本来在回答一个它答不了的问题。
    /// 🚨 「拿开头一段当底噪、按底噪算相对阈值」**被否**：他要是已经开口了，
    ///    那段"底噪"含着他的话 → 底噪估高 → 阈值抬高 → **更容易把他后面的话吃掉**。
    ///    **他越急着说越容易被吃 —— 反馈方向是反的。**
    static let micDeadLevel: Float = 0.01
