...
///
/// 🚨 **这不是改 `History.swift`**：`History` 存的是"已经上屏的一句话"，
///    跟会议转写是不同的数据、不同的生命周期（旁听纪要要能整段删除，
///    History 是永久滚动记录，语义不一样）。
///
/// 🚨🚨 09-18 五订更新：`Segments` 当初**没有**为落盘开口子（"旁听自己包一层，
///    不动共用件"），但录音时长不设上限之后，`MainViewController` 那条录音
///    路径撞上了**同一个**数据丢失窗口——所以给 `Segments` 加了一个**可选**的
///    `onSegmentDone` 回调（默认 nil，旧调用方零行为变化），`MainViewController`
///    传一个闭包进去调 `EavesdropTranscript.append`——**磁盘 I/O 还是调用方自己
///    决定要不要**，`Segments` 本身仍然不直接碰磁盘，这条道理没变，只是从
///    "不留开口"改成了"留一个不强制的开口"。
///
