...
enum WordBookCore {
    ...
    struct Item {
        ...
        /// 复习进度。规则全在 `Srs`。
        let rev: Srs.Rev
        /// **收藏那一刻的卡片**（JSON 原文，可能为空串）。
        ///
        /// 🚨🚨 Kevin 2026-09-06 连问三次「单词卡片在哪儿呢」。
        ///    以前收藏时**把查词已经拿到的解释整份丢掉了** —— 只存了
        ///    词和第一条中文释义，点进去就只有中/英/日期/进度。
        ///
        /// 🚨 **存原文不存解析结果**：两种形态字段不同
        ///    （查词 = senses/examples/collocations；句子 = breakdown/alternatives/keys），
        ///    存成结构体就得先定死一套字段，后端一改就要迁数据。
        /// 🚨 **绝不重新查一次** —— 规格原话：「重新查一次是浪费，而且两次结果
        ///    可能不一样，用户会觉得『我收藏的那个解释变了』」。
        var card: String = ""
    }

    // MARK: - 纯逻辑（不碰存储，所以能进闸门）

    /// **查词条目的身份** —— 只由那个词决定（2.1 09-06 规格）。
    ///
    /// 🚨 小写在**这里**做，不在 `WordId.norm` 里做：
    ///    `norm` 是**跨端契约**（照 Java 那 6 个空白字符写死的），
    ///    在它里面加小写会把**翻译条目的 id 也一起改掉**，两端立刻不一致。
    ///    这条规则只适用于查词，就只放在查词这条路上。
    /// 🚨 收成一个函数：`wbId`（判重）和 `tapAdd`（存）**必须算出同一个值**，
    ///    各写一次的话改口径时必漏一个 —— 表现是"收藏了但按钮没变成已加入"。
    static func dictId(word: String) -> String {
        idOf("", WordId.norm(word).lowercased())
    }
